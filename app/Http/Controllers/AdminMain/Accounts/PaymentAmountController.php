...
class PaymentAmountController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = AccountPaymentAmount::where('company_id', $this->company_id);
        ...
        return view('admin-main.admin.paymentAmount.index', compact('on_accounts', 'parties'));
    }
    ...
    public function create()
    {
        ...
        return view('admin-main.admin.paymentAmount.create', compact(['parties', 'bankDetails']));
    }
    ...
    public function store(Request $request)
    {
        ...
        $invoices = AccountPurchaseInvoice::with('chargesContainer')
         ->where('company_id', $this->company_id)
        ->where('billing_party_id', $request->party_id)
        ->where(function($q) {
        ...
        if($invoices->isNotEmpty()){
            ...
            $data = AccountPaymentAmount::create($validation);
            ...
            return redirect()->route('payment-amount.index')->with('success', 'Payment Amount Data created successfull. !');
        }
        return redirect()->route('payment-amount.index')->with('error', 'Not Found Pending Invoice For This Billing Party. !');
    }
    ...
    {
        $payment_amount = AccountPaymentAmount::where('uuid', $uuid)->firstOrFail();
        $bankDetails = MasterBank::where('company_id', $this->company_id)->get();

        $parties = MasterImportParty::where('company_id', $this->company_id)->where('party_type', 10)->get();
        return view('admin-main.admin.paymentAmount.edit', compact('parties', 'payment_amount', 'bankDetails'));
    }
    ...
    {
        $update_account = AccountPaymentAmount::find($id);
        ...
        if (!empty($request->round_of_amount) && empty($already_round_of_amt_exist)) {
            $invoice = AccountPurchaseInvoice::with('chargesContainer')
                ->where('company_id', $this->company_id)
        ...
        if ($request->amount > $old_amount) {
            ...
            return redirect()->back()->with('success', 'Payment Amount and invoices updated successfully (received amount allocated).');
        }
        ...
        if ($request->amount < $old_amount) {
            ...
            return redirect()->back()->with('success', 'Payment Amount and invoice amounts reversed successfully (amount reduced).');
        }

        
        return redirect()->back()->with('success', 'Payment Amount Data Updated successfull. !');
    }
    ...
    {
        $onAccount = AccountPaymentAmount::findOrFail($id);
        ...
        return redirect()->back()->with('success', 'Payment amount entry deleted and invoice amounts reversed successfully!');
    }
}
