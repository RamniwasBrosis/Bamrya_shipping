...
use App\Models\Accounts\AccountPaymentAmount;
...
class PurchaseOutstandingController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('admin-main.admin.purchaseOutstanding.first', compact('parties', 'branches'));
    }

    public function preview(Request $request)
    {
        $branch_id = $request->branch_id;
        $purchase_invoices = AccountPurchaseInvoice::with(['partyName', 'chargesContainer']) // important
        ...
        $get_round_of_amounts = AccountPaymentAmount::where('company_id', $this->company_id)->where('party_id', $request->party_id)->get();
        $round_of_amount = 0;
           ...
           <div>
                <div class="dropdown mb-3">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'pdf', 'id' => $request->party_id, 'branch_id' => $request->branch_id,]).'" target="_blank">Download PDF</a></li>
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'excel', 'id' => $request->party_id, 'branch_id' => $request->branch_id,]) .'" target="_blank">Download Excel</a></li>
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'word', 'id' => $request->party_id, 'branch_id' => $request->branch_id,]) .'" target="_blank">Download Word</a></li>
                    </ul>
                </div>
           </div>
        ...
        $html .= '<div class="mt-3">' . $purchase_invoices->withQueryString()->links('pagination::bootstrap-5') . '</div>';
    ...
    public function download(Request $request, $format)
    {
        ...
        $purchase_invoices = AccountPurchaseInvoice::with(['partyName', 'chargesContainer'])
        ->where('company_id', $this->company_id)
        ...
        $get_round_of_amounts = AccountPaymentAmount::where('company_id', $this->company_id)->get();
        $round_of_amount = 0;
        $total_get_amount_by_party = 0;
        foreach($get_round_of_amounts as $amount){
            $round_of_amount += $amount->round_of_amount;
        ...
        if ($format == 'pdf') {
            $html = View::make('admin-main.admin.purchaseOutstanding.report', compact('purchase_invoices', 'round_of_amount', 'total_get_amount_by_party'))->render();
        ...
        if ($format == 'excel') {
            return Excel::download(new PurchaseOutstandingExport($purchase_invoices), 'Purchase-Outstanding.xlsx');
        }

        if ($format == 'word') {
            $html = View::make('admin-main.admin.purchaseOutstanding.report', compact('query'))->render();
