...
class PurchaseOutstandingController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin-main.admin.purchaseOutstanding.first', compact('parties'));
    }
           ...
           <div>
                <div class="dropdown mb-3">
                    ...
                    <ul class="dropdown-menu">
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'pdf', 'id' => $request->party_id]).'" target="_blank">Download PDF</a></li>
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'excel', 'id' => $request->party_id]) .'" target="_blank">Download Excel</a></li>
                        <li><a class="dropdown-item" href="'.route('purchase-outstanding.download', ['format' => 'word', 'id' => $request->party_id]) .'" target="_blank">Download Word</a></li>
                    </ul>
                </div>
           </div>
    ...
    public function download($format, $id)
    {
        ...
        $get_round_of_amounts = AccountPaymentAmount::where('company_id', $this->company_id)->where('party_id', $id)->get();
        $round_of_amount = 0;
        $total_get_amount_by_party = 0;
        foreach($get_round_of_amounts as $amount){
            $round_of_amount += $amount->round_of_amount;
        ...
        if ($format == 'excel') {
            return Excel::download(new purchaseOutstandingExport($query), 'Sales-Outstanding.xlsx');
        }
