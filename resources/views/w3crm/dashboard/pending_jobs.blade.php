            ...
            <li class="list-group-item d-flex justify-content-between align-items-center">
                ...
                <div class="d-flex gap-1">
                    @if($job->job_activity == 'SEAIMP.FWD' || $job->job_activity == 'SEAIMP.NVOCC')
                        <a href="{{ route('sea-imports.create', ['job_id' => $job->id]) }}" class="btn btn-sm btn-warning">
                            Create Sea Import
                        </a>
                    @elseif($job->job_activity == 'SEAEXP.FWD' || $job->job_activity == 'SEAEXP.NVOCC')
                        <a href="{{ route('sea-exports.create', ['job_id' => $job->id]) }}" class="btn btn-sm btn-primary">
                            Create Sea Export
                        </a>
                    @elseif($job->job_activity == 'AIRIMP.FWD')
                        <a href="{{ route('air-imports.create', ['job_id' => $job->id]) }}" class="btn btn-sm btn-danger">
                            Create Air Import
                        </a>
