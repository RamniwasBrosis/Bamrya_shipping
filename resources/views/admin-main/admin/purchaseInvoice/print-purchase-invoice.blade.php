...
<body>
    ...
    <div class="invoice-title">{{ $purchaseInvoice->invoice_type ?? 'TAX INVOICE' }}</div>
        ...
        <tr>
            ...
            <td class="header-right">
                ...
                <div class="company-address">
                    ...
                    PHONE: {{ $company->companySetting->phone }} | CIN: {{ $company->companySetting->cin_no }}
    ...
    <table class="info-table" style="margin-bottom:6px;">
        <tr>
            <td class="info-left" rowspan="4">
                ...
                {!! nl2br(e($purchaseInvoice->partyName->address ?? $purchaseInvoice->partyName->address_line1 ?? '')) !!}<br>
                {{ $purchaseInvoice->partyName->city ?? '' }}<br><br>
                GSTIN: {{ $purchaseInvoice->partyName->gstin ?? '' }}
            </td>
            <td class="info-mid"><strong>INVOICE NO:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->invoice_no ?? '' }}</td>
        </tr>
        <tr>
            <td class="info-mid"><strong>DATE:</strong></td>
            <td class="info-right">{{ \Carbon\Carbon::parse($purchaseInvoice->invoice_date)->format('d-m-Y') }}</td>
        </tr>
        <tr>
            <td class="info-mid"><strong>JOB NO:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->operationJob->full_job_no ?? '' }}</td>
        </tr>
        <tr>
            <td class="info-mid"><strong>PORT OF LOADING:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->pol ?? '' }}</td>
        </tr>

        <tr>
            <td class="info-left"><strong>SB/SOE NO:</strong> {{ $purchaseInvoice->shipping_no ?? '' }}</td>
            <td class="info-mid"><strong>PORT OF DELIVERY:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->pod ?? '' }}</td>
        </tr>
        <tr>
            <td class="info-left"><strong>SB/SOE DATE:</strong> {{ \Carbon\Carbon::parse($purchaseInvoice->shipping_bill_date)->format('d-m-Y') }}</td>
            <td class="info-mid"><strong>GROSS WEIGHT:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->gross_weight ?? '' }}</td>
        </tr>
        <tr>
            <td class="info-left"><strong>CBM:</strong> {{ $purchaseInvoice->cbm ?? '' }}</td>
            <td class="info-mid"><strong>NO OF PKGS:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->packages ?? 0 }}</td>
        </tr>
        <tr>
            <td class="info-left"><strong>VESSEL & VOY / AIRLINE:</strong> {{ $purchaseInvoice->vessel_name ?? '' }}</td>
            <td class="info-mid"><strong>CONTAINER NO:</strong></td>
            <td class="info-right">{{ $purchaseInvoice->container_no ?? '' }}</td>
        </tr>
        <tr>
            <td colspan="3"><strong>AWB/BL NO: </strong> {{ $purchaseInvoice->awb_bl_no ?? '' }}</td>
        </tr>
        <tr>
            <td colspan="3"><strong>CONSIGNEE / CONSIGNER: </strong> {{ $purchaseInvoice->consignee ?? '' }}</td>
        </tr>
    </table>
        ...
        <tbody>
            ...
            @foreach ($chargeDetails as $charge)
                ...
                <tr>
                    ...
                    <td class="text-left">{{ $charge->chargeName->charge_name ?? '' }}</td>
    ...
    <table style="width:100%; margin-top:8px;">
        <tr>
            <td style="width:70%;" class="bank-details">
                ...
                ALL CHEQUES/DD TO BE ISSUED IN FAVOUR OF <strong>"{{ $company->company_name }}"</strong>
            </td>
